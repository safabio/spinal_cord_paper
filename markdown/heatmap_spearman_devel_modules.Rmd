...
tmp <- do.call(rbind, col_table) %>% 
  ...
  mutate(sample = paste(sample, "ctrl", sep = "_"))
  select(c("color", "sample_fine"))
```
...
WGCNA_data <- readRDS("~/spinal_cord_paper/output/Gg_devel_int_scWGCNA_010323.rds")
...
my.metam$orig.ident <- str_replace_all(my.metam$orig.ident, pattern =  "ctrl1", "D10_ctrl")

my.metam$sample_celltype <- paste0(substr(my.metam$orig.ident, 1, 3), "_", my.metam$fine)
...
names(my.colsm) <- c("D05_ctrl", "D07_ctrl", "D10_ctrl")
