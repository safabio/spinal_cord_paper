---
title: "comparative scWGCNA v1.0.0 workflow Gg_poly_int with ctrl_int and lumb_int"
author: "Fabio Sacher"
date: "07.02.2023"
output:
  html_document:
...
poly_int_scWGCNA <- readRDS("~/spinal_cord_paper/output/Gg_poly_int_scWGCNA_070223.rds")

# # load module annotation
# mod_annot <- read.delim("~/spinal_cord_paper/data/scWGCNA_Gg_poly_int_module_annotation.txt",sep = ",")
# 
# knitr::kable(mod_annot)

# samples to compare on
Gg_samp <- c("Gg_ctrl_int", "Gg_lumb_int")
...
lumb <- readRDS("~/spinal_cord_paper/data/Gg_lumb_int_seurat_070223.rds")


# concat and name list of samples
samples <- list(ctrl, lumb)
...
Gg_poly_int_comp <- scWGNA.compare(
          scWGCNA.data = poly_int_scWGCNA,
          test.list = ps,
          test.names = Gg_samp,
...
```{r module-frac}
knitr::kable(Gg_poly_int_comp$misc$modulefrac)

# non-orthologous
Gg_poly_int_comp$misc$geneslost[[1]] <- Gg_poly_int_comp$misc$geneslost[[1]] %>%
  dplyr::left_join(gnames, by = "gene")
head(Gg_poly_int_comp$misc$geneslost[[1]])
# non-expressed
Gg_poly_int_comp$misc$geneslost[[2]] <- Gg_poly_int_comp$misc$geneslost[[2]] %>%
  dplyr::left_join(gnames, by = "gene")
head(Gg_poly_int_comp$misc$geneslost[[2]])
...
scW.p.modulefrac.mod(Gg_poly_int_comp)
``` 
...
scW.p.preservation(scWGCNA.comp.data = Gg_poly_int_comp,
                   to.plot=c("preservation", "median.rank"))

scW.p.preservation.mod(scWGCNA.comp.data = Gg_poly_int_comp,
                       pt.style = c(24, 22),
                      to.plot=c("preservation", "median.rank"))
```
...
pdf('~/spinal_cord_paper/figures/Gg_poly_int_comp_scWGCNA.pdf', width = 17)
scW.p.modulefrac.mod(Gg_poly_int_comp)
scW.p.preservation(scWGCNA.comp.data = Gg_poly_int_comp,
                   to.plot=c("preservation", "median.rank"))
...
```{r}
saveRDS(Gg_poly_int_comp, "~/spinal_cord_paper/output/Gg_poly_int_comp_scWGCNA.rds")
```
