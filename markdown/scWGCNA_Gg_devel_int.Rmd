...
library(paste0("org.",my.sp,".eg.db"),character.only = T)
...
for(i in seq(length(names(my.scWGCNA$modules)))){ #as many clusters as we have
   ...
   ex = sapply(dewg[[i]], function(x) AnnotationDbi::exists(x, my.ENS2EG)) #Which genes have an ENTREZ?
    ...
    kewg[[i]] = limma::kegga(dewg[[i]], species = my.sp, universe = unlist(IDs[my.gouni]))
}
...
saveRDS(goterms, paste0("~/spinal_cord_paper/output/", my.se@project.name, "_GOTerms_",format(Sys.Date(), "%d%m%y"),".rds"))
saveRDS(keggpath, paste0("~/spinal_cord_paper/output/", my.se@project.name, "_KEGGPath_",format(Sys.Date(), "%d%m%y"),".rds"))
```
