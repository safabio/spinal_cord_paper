...
For figure for we plot clusters 17, 27 (excitatory neurons), 16, and 18 (MFOL).

```{r}
# vector of exc. neuronal and MFOL clusters
fig_clusters <- c(17, 27, 16, 18)

fig_lumb_markers <- filter(lumb_markers, clust_id %in% fig_clusters)

toplot <- fig_lumb_markers %>% 
  mutate(gene_type = case_when(
    avg_log2FC >= 0.5 & p_val_adj <= 0.05 ~ "ctrl",
    avg_log2FC <= -0.5 & p_val_adj <= 0.05 ~ "lumb",
    TRUE ~ "ns")
...
toplot_nomt <- toplot %>% 
  ...
  filter(!Gene.stable.ID %in% mt) %>% 
  droplevels() %>% 
  mutate(cluster = factor(cluster, levels = c("cl-17","cl-27","cl-16","cl-18")))
...
pdf("~/spinal_cord_paper/figures/Fig_4_volcano.pdf", width = 10, height = 3.5)
volplot_nomt +
  NoLegend() +
    ggrepel::geom_text_repel(aes(label = ifelse(gene_type == 'ns',
                                                NA,
...
```{r}
# Motor neuron cluster
mn.se <- subset(x = my.se, idents = 25)
mn.se$sample <- str_extract(mn.se$orig.ident, "ctrl|poly")
...
VlnPlot(mn.se, group.by = "sample", mn_markers$Gene.stable.ID, cols = c("darkgrey", "goldenrod3"))

mn_DE <- gnames[grepl("NCS1", gnames$Gene.name),]
mn_DE

VlnPlot(mn.se, group.by = "sample", mn_DE$Gene.stable.ID, cols = c("darkgrey", "goldenrod3"))
```
...
We plot cluster 24 and cluster 25 (MFOL late and Motor neurons):

```{r}
# vector of exc. neuronal and MFOL clusters
fig_clusters <- c(24, 25)

fig_poly_markers <- filter(poly_markers, clust_id %in% fig_clusters)

toplot <- fig_poly_markers %>% 
  mutate(gene_type = case_when(
    avg_log2FC >= 0.5 & p_val_adj <= 0.05 ~ "ctrl",
    avg_log2FC <= -0.5 & p_val_adj <= 0.05 ~ "poly",
    TRUE ~ "ns")
...
toplot_nomt <- toplot %>% 
  ...
  # filter(!Gene.stable.ID %in% mt) %>% 
  droplevels() %>% 
  mutate(cluster = factor(cluster, levels = c("cl-24","cl-25")))
...
volplot_nomt <- ggplot(data = toplot_nomt,
  ...
  ylab("-log10(padj)") +
  theme_bw()+
  ggtitle("Ctrl Poly int Sig. marker genes ")
...
pdf("~/spinal_cord_paper/figures/Fig_5_volcano.pdf", width = 3.5, height = 8)
volplot_nomt +
  NoLegend() +
    ggrepel::geom_text_repel(aes(label = ifelse(gene_type == 'ns',
                                                NA,
