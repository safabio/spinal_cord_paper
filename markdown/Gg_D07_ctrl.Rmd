---
title: "Seurat workflow of Gg_NT_D07_ctrl (with SoupX and ddqc)"
author: "Fabio Sacher"
date: "20.01.2023"
output:
  html_document:
...
my.samples[[1]] <- readRDS("~/test/NT_test_soupx_ddqcR/D07_ctrl_ddqc_250123.rds")

#The sample name, will  be used in plots and files
my.samplename <- "Gg_NT_D07_ctrl"
```
...
if (my.multi) {
  my.plots[[5]] = ggplot(emb, aes(x = reduc_1, y = reduc_2)) +
    geom_point(aes(color=sort(meta$orig.ident)), size=2, alpha = 0.4, pch = 19) + 
    scale_colour_manual(values = rainbow(length(table(my.se@meta.data$orig.ident))) ) +
    theme_classic() + labs(colour="Dataset")
}
...
saveRDS(my.se, paste0("~/test/NT_test_soupx_ddqcR/",my.samplename,"_seurat_",format(Sys.Date(), "%d%m%y"),".rds"))

write.table(semarkers, sep = "\t", row.names = T, col.names = T,
            file = paste0("~/test/NT_test_soupx_ddqcR/",my.samplename,"_fullDE_",format(Sys.Date(), "%d%m%y"),".txt"), quote = F)

write.table(semrk10, sep = "\t", row.names = T, col.names = T,
            file = paste0("~/test/NT_test_soupx_ddqcR/",my.samplename,"_top10DE_",format(Sys.Date(), "%d%m%y"),".txt"), quote = F)
