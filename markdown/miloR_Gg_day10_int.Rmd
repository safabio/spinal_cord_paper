---
title: "Milo workflow on Gg_day10_int"
author: "Fabio Sacher"
date: "08.09.225"
output:
  html_document:
...
my.se <- readRDS("~/spinal_cord_paper/data/Gg_day10_int_seurat_090925.rds")

my.se$cond <- my.se$sex
...
my.design <- data.frame(colData(my.milo))[,c("orig.ident", "cond", "batch")] %>% 
  mutate(cond = factor(cond, levels = c("female", "male")))
...
res_2 <- ggplot(da.results, aes(logFC, -log10(SpatialFDR))) + 
  geom_point() +
  geom_hline(yintercept = -log10(0.05)) ## Mark significance threshold (0.05% FDR)
res_2
```
...
DimPlot(my.se,
        reduction = "tsne",
        group.by = "cond") + 
  plotNhoodGraphDA(my.milo, da.results, alpha=1, layout = "TSNE") +
...
DimPlot(my.se,
        reduction = "umap",
        group.by = "cond") + 
  plotNhoodGraphDA(my.milo, da.results, alpha=1, layout = "UMAP") +
  plot_layout(guides="collect")
```
...
da.results$seurat_clusters_mixed <- ifelse(da.results$seurat_clusters_fraction < 0.95, "Mixed", da.results$seurat_clusters)
```

Plot logFC of the nhoods by cell type (cluster):

```{r DA-beeswarm, fig.height= 9, fig.width= 6}
da.results$seurat_clusters_mixed <- factor(da.results$seurat_clusters_mixed, levels = rev(c("Mixed", as.character(1:38))))

plotDAbeeswarm(da.results, group.by = "seurat_clusters_mixed", alpha=1.0)
```

```{r barplot}
bp_ncells_sex <- ggplot(my.se[[]], aes(x = orig.ident, fill = sex)) +
  geom_bar(stat = "count") + 
...
res_2 + ggtitle("line indicates FDR = 0.05 -> no sig. abundance")
bp_ncells_sex
dev.off()

pdf("~/spinal_cord_paper/figures/Gg_day10_int_milo_volplot.pdf", height = 10, width = 5)
plotDAbeeswarm(da.results, group.by = "seurat_clusters_mixed", alpha=1.0) 
dev.off()
